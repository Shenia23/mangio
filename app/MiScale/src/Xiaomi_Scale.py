...
sys.stdout.write('-------------------------------------\n')
sys.stdout.write(
    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Starting Xiaomi mi Scale...\n")
...
try:
    with open('/data/options.json') as json_file:
        sys.stdout.write(
            f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Loading Config From Add-On Options...\n")
        data = json.load(json_file)
        try:
            MISCALE_MAC = data["MISCALE_MAC"]
        except:
            sys.stderr.write(
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - MAC Address not provided...\n")
            raise
        try:
        ...
        try:
            ...
            sys.stderr.write(
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - MQTT Host not provided...\n")
            raise
        try:
        ...
        try:
            ...
            sys.stderr.write(
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - USER1_GT not provided...\n")
            raise
        try:
            USER1_SEX = data["USER1_SEX"]
        except:
            sys.stderr.write(
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - USER1_SEX not provided...\n")
            raise
        try:
            USER1_NAME = data["USER1_NAME"]
        except:
            sys.stderr.write(
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - USER1_NAME not provided...\n")
            raise
        try:
            USER1_HEIGHT = int(data["USER1_HEIGHT"])
        except:
            sys.stderr.write(
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - USER1_HEIGHT not provided...\n")
            raise
        try:
            USER1_DOB = data["USER1_DOB"]
        except:
            sys.stderr.write(
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - USER1_DOB not provided...\n")
            raise
        try:
        ...
        try:
            ...
            pass
        sys.stdout.write(
            f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Config Loaded...\n")
        sys.stdout.write("XIAMOMI", MISCALE_MAC)
# Failed to open options.json, Loading Config From Environment (Not HA Add-On)
except FileNotFoundError:
    pass
    sys.stdout.write(
        f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Loading Config From OS Environment...\n")
    MISCALE_MAC = os.getenv('MISCALE_MAC', '')
    sys.stdout.write(MISCALE_MAC)
    ...
    # User Variables...
    # If the weight is greater than this number, we'll assume that we're weighing User #1
    USER1_GT = int(os.getenv('USER1_GT', '70'))
    USER1_SEX = os.getenv('USER1_SEX', 'male')
    USER1_NAME = os.getenv('USER1_NAME', 'David')  # Name of the user
    USER1_HEIGHT = int(os.getenv('USER1_HEIGHT', '175')
                       )  # Height (in cm) of the user
    # DOB (in yyyy-mm-dd format)
    USER1_DOB = os.getenv('USER1_DOB', '1988-09-30')

    # If the weight is less than this number, we'll assume that we're weighing User #2
    USER2_LT = int(os.getenv('USER2_LT', '55'))
    USER2_SEX = os.getenv('USER2_SEX', 'female')
    USER2_NAME = os.getenv('USER2_NAME', 'Joanne')  # Name of the user
    USER2_HEIGHT = int(os.getenv('USER2_HEIGHT', '155')
                       )  # Height (in cm) of the user
    # DOB (in yyyy-mm-dd format)
    USER2_DOB = os.getenv('USER2_DOB', '1988-10-20')

    USER3_SEX = os.getenv('USER3_SEX', 'male')
    USER3_NAME = os.getenv('USER3_NAME', 'Unknown User')  # Name of the user
    USER3_HEIGHT = int(os.getenv('USER3_HEIGHT', '175')
                       )  # Height (in cm) of the user
    # DOB (in yyyy-mm-dd format)
    USER3_DOB = os.getenv('USER3_DOB', '1988-01-01')
    sys.stdout.write(
        f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Config Loaded...\n")
...
def discovery():
    for MQTTUser in (USER1_NAME, USER2_NAME, USER3_NAME):
        ...
        message += '"state_topic": "' + MQTT_PREFIX + '/' + MQTTUser + \
            '/weight","value_template": "{{ value_json.weight }}",'
        message += '"json_attributes_topic": "' + MQTT_PREFIX + \
            '/' + MQTTUser + '/weight","icon": "mdi:scale-bathroom"}'
        publish.single(
    ...
    sys.stdout.write(
        f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Discovery Completed...\n")
                ...
                if data.startswith('1b18') and sdid == 22:
                    ...
                    if unit and isStabilized:
                        if OLD_MEASURE != round(measured, 2) + int(miimpedance):
                            self._publish(round(measured, 2), unit, str(
                                datetime.now()), hasImpedance, miimpedance)
    ...
    def _publish(self, weight, unit, mitdatetime, hasImpedance, miimpedance):
        ...
        try:
            with open('../../user_data.json', 'w') as f:
            	f.write(message)
            sys.stdout.write(
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Publishing data to topic {MQTT_PREFIX + '/' + user + '/weight'}: {message}\n")
            publish.single(
            ...
            sys.stdout.write(
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Data Published ...\n")
        except Exception as error:
            sys.stderr.write(
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Could not publish to MQTT: {error}\n")
            raise
...
def main():
    ...
    while True:
        try:
            ...
            # Adding passive=True to try and fix issues on RPi devices
            scanner.scan(5)
        except BTLEDisconnectError as error:
            sys.stderr.write(
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - btle disconnected: {error}\n")
            pass
        except BTLEManagementError as error:
            sys.stderr.write(
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Bluetooth connection error: {error}\n")
            if BluetoothFailCounter >= 4:
                sys.stderr.write(
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - 5+ Bluetooth connection errors. Resetting Bluetooth...\n")
            ...
            sys.stderr.write(
                f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Error while running the script: {error}\n")
            pass
        else:
            BluetoothFailCounter = 0
...
if __name__ == "__main__":
    main()
    sys.stdout.write('MISCALE', MISCALE_MAC)
